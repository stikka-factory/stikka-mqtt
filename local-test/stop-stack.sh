...
pkill -f "mosquitto -c $ROOT_DIR/local-test/mosquitto.conf" 2>/dev/null || true
pkill -f "esp32/tools/mock_bridge_server.py" 2>/dev/null || true

rm -f "$LOCK_FILE"

echo "[local-test] remaining listeners on 5173/1883/9001:"
ss -ltn '( sport = :5173 or sport = :1883 or sport = :9001 )' || true

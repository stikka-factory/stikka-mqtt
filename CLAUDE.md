...
| `esp32/src/mqtt_bridge.h/.cpp` | MQTT connect, `/<printer>/command/`+`/<printer>/status/` topics, status publishing, command JSON parsing (ZPL `utf8`/`base64_utf8`, image `base64_png`/`data_url`, and `ql_raster` `base64_bytes` — every job a single MQTT message, no chunking protocol), dispatch straight to the compiled-in target. `PROTOCOL_QL` builds only accept `payload_type: "ql_raster"` (pre-rasterized by the frontend, see `zpl-image.ts` below) and reject `"zpl"`/`"image"`; non-QL builds keep accepting `"zpl"`/`"image"` and reject `"ql_raster"`. Uses [espMqttClient](https://github.com/bertmelis/espMqttClient), not PubSubClient — PubSubClient required one contiguous receive buffer sized up front (hard-capped at 65535 bytes, its `bufferSize` field being a `uint16_t`); espMqttClient instead streams a PUBLISH's payload to the message callback through a small fixed ~1.4KB buffer regardless of total message size, reassembled in `mqtt_bridge.cpp`, so there's no more library-imposed per-message ceiling |
...
| `esp32/src/targets/target.h/.cpp` | Shared target contract (`targetSend`/`targetSendString`/`targetSetup`/`targetMethodName`) plus a write-loop-with-timeout helper shared by the serial/usb targets |
...
**Current device / default build target**: ESP32-S3-DevKitC-1 (N16R8 module — 16MB flash + 8MB octal-SPI PSRAM; `default_envs = esp32-s3-devkitc-1_zpl_network` in `platformio.ini`) — the only supported board  
**Build**: `pio run` from `esp32/` (builds every active/uncommented env), or `pio run -e esp32-s3-devkitc-1_zpl_network -t upload` to flash a specific one — two leaf envs currently defined: `esp32-s3-devkitc-1_zpl_network` (default), `esp32-s3-devkitc-1_ql_usb_host`  
**PSRAM (required)**: `board_build.arduino.memory_type = qio_opi` in `platformio.ini` enables the S3's octal PSRAM. ESP-IDF routes large mallocs/reallocs (the reassembled JSON `String`, then its base64-decoded twin) to PSRAM automatically once it's enabled, so a whole job fits in memory at once regardless of label size — every job goes out and is processed as a single MQTT message, there is no chunking protocol (see "MQTT Message Contract" below)  
...
- ESP32 firmware's MQTT client (espMqttClient) has no fixed per-message packet-size ceiling — it streams a PUBLISH's payload through a small fixed buffer regardless of total size (replaces PubSubClient's old hard 65535-byte `uint16_t` `bufferSize` cap). Every job is sent and processed as a single MQTT message (`esp32/src/mqtt_bridge.cpp`, `mqtt-client.ts`); PSRAM (required, `esp32-s3-devkitc-1` only) gives the board enough heap headroom to hold any realistic job in memory at once
...
Every job is sent and processed as a single MQTT message — there is no chunking protocol. This relies on PSRAM (required, `esp32-s3-devkitc-1` only): the ESP32 MQTT client (espMqttClient) streams a message's payload through a small fixed internal buffer regardless of total size (`mqtt_bridge.cpp`'s raw-message reassembly, unrelated to payload chunking), and ESP-IDF routes the reassembled job's large allocations (JSON text, then its base64-decoded twin) to PSRAM automatically once it's enabled, so a whole job fits in memory at once regardless of label size. ZPL is sent as plain `utf8` (no base64 wrapping — ZPL is already ASCII-safe JSON text, and base64 would cost 33% for nothing; `base64_utf8` also exists but is currently unused by the frontend); image bytes are `base64_png`/`data_url`; `ql_raster` bytes are `base64_bytes`. `ql_raster` carries the already-rasterized Brother QL byte stream (built client-side by `zpl-image.ts`, see `esp32/README.md`'s "Brother QL raster protocol" section) — `PROTOCOL_QL` firmware builds only accept this payload_type, not `image`/`zpl`. The firmware forwards whatever it decodes straight to the target without further processing (image/zpl on non-QL builds, ql_raster on QL builds).

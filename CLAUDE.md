...
**PSRAM boards**: `board_build.arduino.memory_type = qio_opi` in `platformio.ini` enables the S3's octal PSRAM. espMqttClient's own receive buffer is a small fixed size regardless of PSRAM (see the `mqtt_bridge.h/.cpp` row above), so PSRAM's benefit now is purely headroom for the *reassembled job itself* — ESP-IDF routes large mallocs/reallocs (the reassembled JSON `String`, then its base64-decoded twin) to PSRAM automatically once it's enabled, no separate allocation path needed. `mqtt_bridge.cpp` reports that headroom to the frontend as `capabilities.maxPayloadBytes` (a live fraction of `ESP.getMaxAllocHeap()`), which `mqtt-client.ts` uses to skip chunking for jobs that fit — see "MQTT Message Contract" below  
...
- ESP32 firmware's MQTT client (espMqttClient) has no fixed per-message packet-size ceiling — it streams a PUBLISH's payload through a small fixed buffer regardless of total size (replaces PubSubClient's old hard 65535-byte `uint16_t` `bufferSize` cap). The real ceiling is now board heap, reported per printer as `capabilities.maxPayloadBytes`; jobs above what a printer actually reports must still be chunked client-side (`esp32/src/mqtt_bridge.cpp`, `mqtt-client.ts`)
...
Large image/ZPL/ql_raster payloads are split across multiple messages using the `*_chunk` encodings + `chunk_index`/`chunks_total`, once the payload exceeds a **per-printer** threshold in `mqtt-client.ts` — not one fixed constant. Each printer reports its own heap-based headroom for a whole command message as `capabilities.maxPayloadBytes` (`maxCommandPayloadBytes()` in `mqtt_bridge.cpp`, a live fraction of `ESP.getMaxAllocHeap()` — the ESP32 MQTT client, espMqttClient, streams a message's payload through a small fixed internal buffer regardless of total size, so heap for the reassembled job is the real constraint now, not a negotiated packet-size ceiling); the frontend caches that per printer name (from status broadcasts, not persisted to Supabase) and only chunks a job that exceeds it, falling back to the original `IMAGE_CHUNK_SIZE`/`ZPL_CHUNK_SIZE`/`QL_RASTER_CHUNK_SIZE` constants (8000 bytes each) for a printer it hasn't heard from yet or older firmware that doesn't report the field. A non-PSRAM board (`m5stack-atom`) has much less heap headroom and so still chunks around 8000 bytes as before; a PSRAM board (`esp32-s3-devkitc-1`) reports a much larger figure and so skips chunking for most real-world jobs (see the PSRAM note above) — the reassembled job (JSON text, then its base64-decoded twin) stays small/DRAM-safe on non-PSRAM boards, and is routed to PSRAM automatically on boards that have it. ZPL is sent as plain `utf8`/`utf8_chunk` (no base64 wrapping — ZPL is already ASCII-safe JSON text, and base64 would cost 33% for nothing); image and `ql_raster` bytes stay `base64_png`/`base64_bytes`/`base64_chunk` since they're binary. `ql_raster` carries the already-rasterized Brother QL byte stream (built client-side by `zpl-image.ts`, see `esp32/README.md`'s "Brother QL raster protocol" section) — `PROTOCOL_QL` firmware builds only accept this payload_type, not `image`/`zpl`. The firmware forwards whatever it reassembles straight to the target without decoding it (image/zpl on non-QL builds, ql_raster on QL builds).
